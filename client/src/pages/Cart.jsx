              ...
              <div className="flex flex-1 flex-col justify-between">
                <div>
                  <div className="flex justify-between gap-4">
                    <Link to={`/products/${item.id}`} className="text-sm font-medium hover:opacity-55">
                      {item.name}
                    </Link>
                    <span className="text-sm">{formatPrice(item.price * item.quantity)}</span>
                  </div>
                  <p className="mt-1 text-xs text-muted">{item.colour} · Size {item.size}</p>
                </div>
                ...
                <div className="mt-4 flex items-center justify-between">
                  <div className="flex items-center border border-line">
                    <button
                      ...
                      className="px-3 py-1.5 text-sm hover:bg-line"
                    >
                      −
                    </button>
                    <span className="min-w-[36px] text-center text-sm">{item.quantity}</span>
                    <button
                      onClick={() => updateQuantity(item.key, item.quantity + 1)}
                      className="px-3 py-1.5 text-sm hover:bg-line"
                    >
